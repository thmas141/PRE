...
def plot_SA(output_file) :
    ...
    Y1=ns.Get_Data_Columns(output_file,1)
    Y2=ns.Get_Data_Columns(output_file,2)
    Y3=ns.Get_Data_Columns(output_file,3)#be carefull : behind or up is not the same
...
def antenna(t,output_file,DM,defects):
    ...
    if defects : 
        ...
        setup_defect(AFM,Lx,Ly,Lz,50*cellsize,50*cellsize,cellsize)        
    H0=0
    He=2e5  
    ns.setstage('Hequation')
    ...
    ns.editdatasave(0,'time',time_step)
    Datasave = [['time'],['<M>', AFM, [Lx*0.75*1e-9,0,0,Lx*0.8*1e-9,Ly*1e-9,Lz*1e-9]]]
    pos_defect = [1850*1e-9, 140*1e-9, 0]
    for i in range(0,30):
        for j in range(0,30):
            #Datasave.append(['<Mz>', AFM, [pos_defect[0]+i*cellsize*1e-9, pos_defect[1]+j*cellsize*1e-9, 0, pos_defect[0]+(i+1)*cellsize*1e-9, pos_defect[1]+(j+1)*cellsize*1e-9, Lz*1e-9]])
            Datasave.append(['<My>', AFM, [0.5*Lx*1e-9*(1+i/31),Ly*(j/31)*1e-9, 0, 0.5*Lx*1e-9*(1+(i+1)/31), Ly*1e-9*((j+1)/31), Lz*1e-9]])
    ns.setsavedata(output_file, *Datasave)

    ns.Run()

def antenna_plot(output_file,Lx,Ly,Lz,cellsize,nb,savename):
    X=ns.Get_Data_Columns(output_file,0)
    def f(m,n):
        mz=ns.Get_Data_Columns(output_file,2+30*m+n)[nb]
        return mz/3500 #np.mean(mz[len(mz)//2:])
...
def antenna_bulk(t,output_file,DM,defects):
    time_step=t*1e-12*1e-4# for gif
    Lx=1500
    Ly=600
    Lz=100
    ...
    if DM :
        modules.append('DMexchange')
    file=Path(output_file)
    AFM=init('FeBO3',Lx,Ly,Lz,cellsize,T,modules,30,0,defects)#remember to add bool defect
    ns.setdt(1e-14)
    if defects : 
        #AFM.param.Ms_AFM.setparamvar(['defects',0,1e-2,8*cellsize*1e-9,8*cellsize*1e-9,1500*1e-9,100])#AFM.param.Ms_AFM.setparamvar(['defects',0,1e-2,8*cellsize*1e-9,8*cellsize*1e-9,1500*1e-9,100]) max
        setup_defect(AFM,Lx,Ly,Lz,3*cellsize,3*cellsize,2*cellsize,10)       
    H0=0
    He=3e5 
    ns.setstage('Hequation')
    #ns.equationconstants('u', t)
...
def fit_sine_sum(file, column, nb_terms=2):
    ...
    t_new = t[(len(t) * 6) // 10:]/3500
    y_new = y[(len(t) * 6) // 10:]/3500
...
def plot_fourier_xyz(output_file, place='above', nb_peaks=5):
    ...
    for i, (label, signal) in enumerate(components.items()):
        ...
        #     width = 3  # ex. supprime 5 points avant et après
        #     start_idx = max(0, max_peak_idx - width)
        #     end_idx = min(len(magnitudes), max_peak_idx + width + 1)
        #     magnitudes[start_idx:end_idx] = 0
...
def antenna_ovf2(t,output_file,DM,defects,f):
    time_step=t*1e-12*1e-4# for gif
    Lx=1500
    Ly=600
    Lz=100
    ...
    while time<t:
        for fname in sorted(os.listdir(basepath)):
            if not fname.endswith('.ovf'): continue
            fullfn = os.path.join(basepath, fname)

            # 1) Load this OVF2 as the magnetic (Zeeman) field:
            ns.loadovf2field('afm', fullfn)

            # 2) Recompute fields so the new Zeeman term is applied:
            ns.computefields()

            # 3) Advance your magnetization dynamics one time-step (or Relax/Run):
            ns.Relax(['time',['time', (1/f)*1e-12]])    # or ns.relax([...]) depending on your script
        time+=100/f
...
def plot_field(ovf_filename, z0,defect_pos=None):
    ...
    plt.xlabel('x (m)')
    plt.ylabel('y (m)')
    plt.colorbar(label='H (T)',shrink=0.2)
...
def bulk_analysis_stray(t,ovf_filename,output_file,DM,defects):
    ...
    if DM :
        modules.append('DMexchange')
    AFM=init('FeBO3',Lx,Ly,Lz,cellsize,T,modules,0,0)
    ns.setdt(10e-15)
###############test taille supermesh
    ...
    if defects : 
        #setup_defect(AFM,Lx,Ly,Lz,5*cellsize,5*cellsize,0.5*Lz)  
        setup_defect(AFM,Lx,Ly,Lz,2*cellsize,2*cellsize,2*cellsize,2)        
    He=6e5
    H0=0
    ns.setstage('Hequation')
    ns.equationconstants('H0', H0)
    ...
    #     ns.reset()
    #     ns.editstagestop(0, 'time', 1*1e-12)
    #     ns.Run()
    #     ns.computefields()
    #     ns.saveovf2(Vis, 'Heff', ovf_filename)


def plot_field2(ovf_filename, z0, defect_pos=None):
    # Read OVF2 file
    vec, nodes, rect = ns.Read_OVF2(ovf_filename)
    Nx, Ny, Nz = nodes  # number of cells in x, y, z
    ...
    plt.xlabel('x (m)')
    plt.ylabel('y (m)')
    plt.colorbar(label='H (T)',shrink=0.5)
...
def main(simu,material,DMI=False):
    ...
    #magnon_transport_SOT (material,Lx,Ly,Lz,cellsize,0,['exchange', 'anitens'],damping_x,0,0,1000,' ')
        ...
        # DM=False
        t=300
        # output_file='C:/Users/thdelvau/OneDrive - NTNU/Documents/resultst3/antenna.txt'
        # if os.path.exists(output_file):
        #     os.remove(output_file)
        # antenna(t,output_file,DMI,True)
        #plot_SA(output_file)
        output_file='C:/Users/thdelvau/OneDrive - NTNU/Documents/resukts4/jsp/antenna-Mx.txt'
        for i in range (1,10):
            savename='C:/Users/thdelvau/OneDrive - NTNU/Documents/resukts4/jsp/Mx/'+str(i)+'.png'
            antenna_plot(output_file,Lx,Ly,Lz,cellsize,-10*i,savename)
        ...
        defect=True
        t=900
        output_file='C:/Users/thdelvau/OneDrive - NTNU/Documents/results/4/bulk_analysis/10_def_bottom_analysis.txt'
        if os.path.exists(output_file):
             os.remove(output_file)
        antenna_bulk(t,output_file,DM,defect)
        plot_SA(output_file)
        plot_fourier_xyz(output_file)
        #plot_fit_sine_sum(output_file,'above',3)
    elif(simu=='bulkDMI'):
        #output_file="C:/Users/thdelvau/OneDrive - NTNU/Documents/results/3/bulk/def_0.5Lz/nothing.txt" #nothing
        #output_file="C:/Users/thdelvau/OneDrive - NTNU/Documents/results/3/bulk/def_0.5Lz/def.txt" #1def
        #output_file="C:/Users/thdelvau/OneDrive - NTNU/Documents/results/5/DM+1def+3D/data.txt" #1def+DMI
        #output_file="C:/Users/thdelvau/OneDrive - NTNU/Documents/results/5/DM+5def/3D/backup/data.txt" #5def+DMI
        #output_file="C:/Users/thdelvau/OneDrive - NTNU/Documents/results/5/DM+10def/3D/data.txt" #10def+DMI
        #output_file="C:/Users/thdelvau/OneDrive - NTNU/Documents/results/5/10def/data.txt" #10def
        ovf_filename="C:/Users/thdelvau/OneDrive - NTNU/Documents/results/5/10def/stray.ovf"
        ...
        # basepath='C:/Users/thdelvau/OneDrive - NTNU/Documents/results/5/DM+2def/'
        # ovf_filename = os.path.join(basepath, "straynoDM.ovf")
        # DM=False
        # defects=True
        # t=900
        # bulk_analysis_stray(t,ovf_filename,output_file,DM,defects)
        #plot_SA(output_file)
        #plot_fourier_xyz(output_file)
        plot_field(ovf_filename,(-5)*1e-9,(0.29, 0.5))
        # plot_field(ovf_filename,(-10)*1e-9,(0.29, 0.5))
        # plot_field(ovf_filename,(-20)*1e-9,(0.29, 0.5))
        # plot_field(ovf_filename,(-50)*1e-9,(0.29, 0.5))
main('bulkDMI','FeBO3',True)
